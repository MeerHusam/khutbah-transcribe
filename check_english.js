...
const arWords = s => normalizeArabic(s ?? '').replace(/[^ء-ي\s]/g, ' ').split(/\s+/).filter(Boolean);
...
export function checkEnglish(blocks, result) {
  ...
  for (const b of blocks) {
    ...
    const claudeSkel = new Set(claude.split(/\s+/).map(skeleton).filter(Boolean));
    ...
    for (const m of prose.matchAll(QUOTE_RE)) {
      ...
      const at = `${src.kind} ${src.label}: “${quote.slice(0, 60)}${quote.length > 60 ? '…' : ''}”`;
      ...
      if (added.length) failures.push(`swapped ${at} adds ${added.map(w => `"${w}"`).join(', ')}, not in Claude's translation of the block`);
      ...
      if (QUOTE_OPENS_WITH_PROPHET.test(quote) && PROPHET.test(before)) {
        failures.push(`swapped ${at} names the Prophet again right after the imam's own "The Prophet … said"`);
      }
      if (ENDS_ON_FRAMING.test(quote.replace(/[.,;:!?'’\s]+$/, ''))) {
        failures.push(`swapped ${at} ends mid-sentence on its own framing ("…${quote.trim().split(/\s+/).slice(-3).join(' ')}")`);
      }
      if (/\?\?/.test(quote)) failures.push(`swapped ${at} carries sunnah.com's "[??]" placeholder`);
      ...
      if (src.kind === 'hadith' && src.arabic) {
        ...
        if (miss.length) failures.push(`swapped ${at} comes from a text without the imam's "${miss.join(' ')}"`);
      }
      ...
      if (src.kind === 'verse') {
        ...
        if (enFrac - arFrac > 0.3 && (enFrac - arFrac) * src.keys.length >= 5) {
          failures.push(`swapped ${at} covers ${Math.round(enFrac * 100)}% of the verse's English but the imam recited ${Math.round(arFrac * 100)}% of it`);
        } else if (arFrac - enFrac > 0.4 && (arFrac - enFrac) * src.keys.length >= 5) {
          failures.push(`swapped ${at} covers ${Math.round(enFrac * 100)}% of the verse's English but the imam recited ${Math.round(arFrac * 100)}% of it`);
        }
      }
    }
